document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchBalance() {
        try {
            const response = await fetch(`http://localhost:8080/game/balance?telegramId=${telegramId}`);
            if (!response.ok) {
    ...
    async function flipCoin(choice) {
        ...
        try {
            const response = await fetch(`http://localhost:8080/game/play?telegramId=${telegramId}&bet=${bet}`, {
                method: 'POST',
                headers: {
